...
- **Git コマンドガイド**: クローン → ブランチ作成 → プッシュまでの手順を、コピーボタン付きコマンドカードで段階的に提示
- **管理者ダッシュボード**: メンバーの進捗管理・プロジェクト作成・GitHub Issue 連携を一元管理
- **学習モジュール**: HTML/CSS・Git/GitHub など初心者向け学習コンテンツを提供。オンボーディング資料は管理者がマークダウンエディタで編集
- **GitHub OAuth ログイン**: Firebase Authentication によるソーシャルログイン
- **デモモード**: 環境変数が未設定の場合はモックデータで起動し、Firebase や API キーなしで UI を確認可能
